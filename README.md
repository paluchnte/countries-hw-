...
1. `python3 -m venv venv_name` - запуск виртуального окружения.
2. `source venv_name/bin/activate` - активация виртуального окружения.
3. `pip install -r requirements.txt` - подключение всех библиотек проекта.
4. `python manage.py migrate` - создание базу данных.
5. `python manage.py runserver` - запуск серверв.
6. `pip install freeze > requirements.txt` - обновиление списка библиотек по окончанию проекта.
